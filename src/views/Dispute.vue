...
<template>
  <div class="w-full gap-2 lg:gap-0 h-[80dvh] flex flex-col justify-between items-center">
    ...
    <div class="flex flex-wrap gap-2 lg:gap-0 lg:flex-nowrap w-full justify-between h-fit lg:h-[5%]">
      <button
        class="pointer grow lg:grow-0 py-2 w-52 rounded-lg border border-secondary bg-secondary"
        @click="router.go(-1)"
      >
        Back
      </button>
      <div class="flex flex-wrap lg:flex-wrap gap-2 w-fit">
        <button
        @click="disputeAnswer = 1"
          class="pointer grow lg:grow-0 py-2 w-52 rounded-lg border border-secondary bg-valid"
        >
          Accept Dispute
        </button>
        <button
        @click="disputeAnswer = 2"
          class="pointer grow lg:grow-0 py-2 w-52 rounded-lg border border-secondary bg-invalid"
        >
          Reject Dispute
        </button>
      </div>
    ...
    <div v-if="disputeAnswer != 0" class="absolute z-10 w-dvw h-dvh top-0 left-0 flex items-center justify-center">
      ...
      <div class="bg-secondary absolute rounded-lg border border-accent flex flex-col items-center justify-center px-8 py-10 gap-2">
        ...
        <p class="w-2/3 text-center">This action is permanent and cannot be undone. (Action: <strong>{{ disputeAnswer == 1 ? 'Approve' : 'Reject' }} Dispute</strong>)</p>
        <div class="flex gap-2 justify-start items-center w-2/3">
          <input v-model="checkbox1" type="checkbox">
          <label>I have read through the dispute form</label>
        </div>
        <div class="flex gap-2 justify-start items-center w-2/3">
          <input v-model="checkbox2" type="checkbox">
          <label>I have read and checked task details</label>
        </div>
        <div class="flex gap-2 justify-start items-center w-2/3">
          <input v-model="checkbox3" type="checkbox">
          <label>I have explained my reasoning in the Admin Notes section</label>
        </div>
